...
setupMap = function () {
  ...
  ctx.strokeStyle = '#0ac';
  ctx.stroke();
  drawPlanet(6, 8, 7, '#f41'); // red, blue, purple, orange, green, yellow
  drawPlanet(7, 6, 8, '#14f');
  drawPlanet(8, 7, 6, '#e2f');

  drawPlanet(3, 2, 1, '#e60');
  drawPlanet(1, 3, 2, '#0a4');
...
drawPlanet = function (x, y, z, color) {
  ...
  plane.style.right = '56px';
  plane.style.transform = 'translateZ(' + (-86) * z + 'px)';
  plane.height = 500;
  plane.width = 860;
  plane.style.height = '500px';
  plane.style.width = '860px';

  ctx.moveTo(86 * x, 500);
  ctx.lineTo(86 * x, 520 - 50 * y);
  ctx.strokeStyle = color;
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(86 * x, 500 - 50 * y, 20, 1.15 * 2*Math.PI, 0.6 * 2*Math.PI);
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(86 * x, 500 - 50 * y, 10, 0, 2*Math.PI);
  ctx.fillStyle = color;
  ctx.fill();
